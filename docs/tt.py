# Name:         tkintertoy.py - Mike Callahan - Python 2.7 or 3.4 2/11/2019
...
class Window(object):
    ...
    def addSpin(self, tag, parms, between='', prompt='', **tkparms):
        ...
        The get method returns a str, but set requires a list of int.
        ...
        self.content[tag] = {'type':'spin', 'value':[], 'between':between} # data is list
        if prompt:
            frame = ttk.LabelFrame(self.master, text=prompt) # create titled frame
        else:
            frame = ttk.Frame(self.master)             # no title
    ...
    def addCollector(self, tag, height, columns, widgets, prompt='', **tkparms):
        ...
        This group of widgets allows the programmer to collect the contents
    ...
    def addSizegrip(self, tag, **tkparms):
        ...
        usually not needed since most platforms add this automatically. The
        programmer must use the configurerow and configurecolumn options when
        plotting widgets for this to work correctly. There is no frame.
    ...
    def get(self, tag, getValues=False):
        ...
        if widgetType in ('label', 'message'):
            ...
            between = self.content[tag].get('between')
            #print('between:',between)
            value = ''                                 # get returns a str
            for item in self.content[tag]['value']:
                value += str(item.get())               # get all tk variables
                value += between
                #print('value:',value)
            if between:
                value = value[:-1]                     # delete final between
